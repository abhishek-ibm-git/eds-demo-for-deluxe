export default function decorate(block) {
  ...
  block.classList.add(`columns-infographic-${cols.length}-cols`);
    ...
    [...row.children].forEach((col) => {
      ...
      if (pic) {
        ...
        if (picWrapper && picWrapper.children.length === 1) {
          ...
          picWrapper.classList.add('columns-infographic-img-col');
        }
      } else {
        ...
        if (img) {
          ...
          if (onlyChild) {
            col.classList.add('columns-infographic-img-col');
          }
        }
      }
    });
